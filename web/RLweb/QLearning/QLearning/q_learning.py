...
class Agent:
    ...
    def __init__(self, R, goal, alpha=0.8, gamma=0.8, tau=2000.0, epsilon=0.7):
        self.R = R
        self.state = 0  # int
        ...
        if Agent.Q is None:
            Agent.Q = np.zeros_like(self.R, dtype=np.float)
    ...
    def recognize_possible_action(self, state):
        ...
        print(np.where(self.R[state, :] != -1)[0])
    ...
    def _soft_epsilon_greedy(self):
        ...
        q_list_np = np.array(q_list) / self.tau
...
def q_learn(episode_num, R, goal, gamma, alpha, tau, epsilon):
    Agent_dic = {}
    ...
    return Agent_dic['agent{}'.format(i)].Q
